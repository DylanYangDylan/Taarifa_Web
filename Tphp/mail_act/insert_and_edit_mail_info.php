...
if($type=='0'){
		...
		$strSqlCommand_email_send = "UPDATE  `db3`.`incident` SET  `email_send` =  '1' WHERE  `incident`.`id` =".$incident_id;
		...
		$strSqlCommand ="UPDATE mail_info SET msg='".$msg."',mail_to='".$mail_to."',sub='".$sub."',time='".date("Y-m-d H:i:s",strtotime("now"))."' WHERE incident_id=".$incident_id;
...
function sendmail($mail_to, $sub, $msg){
	...
	$mail->Host = "localhost";   
	$mail->SMTPAuth = true;                           
	$mail->Username = 'young@ghdistrictsmonitor.org';                            
	$mail->Password = '123465';     
	$mail->SMTPDebug = 0;                        
