...
<!--  ================================================= Group For Admin =================================================  -->
					...
					$result18 = mysql_query($strSqlCommand1);
                	...
                	<tbody>
							...
							<?php
								foreach ($incidents as $incident)
								{
									...
									$strSqlCommand8 = "SELECT `ggroup_id` FROM `incident` WHERE `id`= ".$incident_id;
									$result8 = mysql_query($strSqlCommand8);
									$row8 = mysql_fetch_array ($result8);
									
									if($row8['ggroup_id']==$row18['id']){
                                       ...
                                       <?php 
												...
												if($num_rows > 0){
													...
													while($row9 = mysql_fetch_array ($result9)){
														if($count==1){
	                        								echo "<a class='photothumb' rel='lightbox-group".$incident_id."' href='http://localhost:8888/media/uploads/".$row9['media_link']."'>";
	                        								echo "<img style='width:100%;width:103px\9;height:106px;' src='http://localhost:8888/media/uploads/".$row9['media_link']."'/>";
	                        								echo "</a>";
															//echo "<br>";
														}
														else{
															echo "<a class='photothumb' rel='lightbox-group".$incident_id."' href='http://localhost:8888/media/uploads/".$row9['media_link']."'>";
	                        								echo "<img style='display:none; src='http://localhost:8888/media/uploads/".$row9['media_link']."'/>";
	                        								echo "</a>";
															//echo "<br>";
														}
														$count++;
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th class="col-2" style="width:350px;"><?php echo Kohana::lang('ui_main.report_details');?></th>

                                	<th class="col-4" style="width:100px;text-align:center;"><?php echo Kohana::lang('ui_main.actions');?></th>
									<th class="col-3" style="text-align:center;"><?php echo Kohana::lang('ui_admin.group_photo');?></th>
							...
							<?php
								foreach ($incidents as $incident)
								{
									...
									$strSqlCommand7 = "SELECT `ggroup_id` FROM `incident` WHERE `id`= ".$incident_id;
									...
									<tr>
										...
										<td class="col-2">
											<div class="post">
												<h4>
													<a id="title_<?php echo $incident_id; ?>" href="<?php echo url::site() . 'admin/reports/edit/' . $incident_id; ?>" class="more">
														<?php echo $incident_title; ?>
													</a><p style="float:right"><?php echo $incident_date; ?></p>
												</h4>
												<p><?php echo $incident_description; ?>... 
                         ...
                         -->
							...
							<?php
								foreach ($incidents as $incident)
								{
									...
									//add user_id to distinct user power 
									$strSqlCommand7 = "SELECT `ggroup_id`,`user_id` FROM `incident` WHERE `id`= ".$incident_id;
									$result7 = mysql_query($strSqlCommand7);
									$row = mysql_fetch_array ($result7);
									
									//add user_id to distinct user power
									if($row['ggroup_id'] == 0 && $row['user_id'] == $this->user->id){
...
function setInfoToForm(id){
		...
		check=$('#check_has_email_input').val();
			...
			if(check=="0"){
					...
					$('#mail_msg').val("Please view the report here<br> http://ghdistrictsmonitor.org/index.php/reports/view/"+id);
					 blockUI();
